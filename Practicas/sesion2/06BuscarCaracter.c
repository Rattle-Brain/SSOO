...
int main (int argc, char *argv[]) {
  ...
  if (argc==1)
    ...
    if (argc==2)
      ...
      buscarYContar(argv[POSICION_CARACTER_EN_LINEA_COMANDOS], argv[POSICION_CADENA_EN_LINEA_COMANDOS]);
  exit(1);
}
...
void buscarYContar(char caracter, char *cadena) {
  ...
  printf("El numero de caracteres '%c' encontrado fue %d\n", caracter, contadorCaracteres);
}
