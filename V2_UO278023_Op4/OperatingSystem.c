...
//Function declaration for the exam
...
void OperatingSystem_WakeUpProcess()
{
		...
		&& processTable[Heap_getFirst(sleepingProcessesQueue, numberOfSleepingProcesses)].whenToWakeUp == interrupts)
	{
		...
		processTable[sleepingPID].whenToWakeUp = Clock_GetTime() - processTable[sleepingPID].whenToWakeUp;
		processTable[sleepingPID].priority = processTable[sleepingPID].priority - processTable[sleepingPID].whenToWakeUp;
		if(processTable[sleepingPID].priority < 1)
		{
			processTable[sleepingPID].priority = 1;
		}
...
void OperatingSystem_SendProcessToSleep()
{
	...
	processTable[PID].ticksAsleep = Clock_GetTime();
