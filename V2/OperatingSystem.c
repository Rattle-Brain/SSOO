...
int OperatingSystem_LongTermScheduler() {
	...
	for (i=0; programList[i]!=NULL && i<PROGRAMSMAXNUMBER ; i++) {
		...
		processTable[PID].responseTime = Clock_GetTime();
		processTable[PID].waitingTime = Clock_GetTime();
		processTable[PID].returnTime = Clock_GetTime();
...
void OperatingSystem_Dispatch(int PID) {
	...
	processTable[PID].waitingTime += (Clock_GetTime() - processTable[PID].responseTime);
	processTable[PID].state=EXECUTING;
	if(processTable[PID].timesOnProcessor == 0)
	{
		processTable[PID].responseTime = Clock_GetTime() - processTable[PID].responseTime;
		processTable[PID].timesOnProcessor++;
	}
...
void OperatingSystem_TerminateProcess() {
	...
	processTable[executingProcessID].returnTime = Clock_GetTime() - processTable[executingProcessID].returnTime;
	processTable[executingProcessID].state=EXIT;
	char *nombre = programList[processTable[executingProcessID].programListIndex]->executableName;
	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(550, SYSPROC, "response time", executingProcessID, nombre, processTable[executingProcessID].responseTime);
	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(550, SYSPROC, "return time", executingProcessID, nombre, processTable[executingProcessID].returnTime);
	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(550, SYSPROC, "wating Ttime", executingProcessID, nombre, processTable[executingProcessID].waitingTime);
